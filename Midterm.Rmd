...
#chemical related value in each chemical type
...
map <-function(years = "2019",state = "CALIFORNIA", chemical = "NOP USDA CERTIFIED", measurement = "MEASURED IN LB"){
usmap::plot_usmap(data = strawb1[strawb1$year==years& strawb1$state == state& strawb1$chemical == chemical & strawb1$`measurement.s.` == measurement,], values = "value", color = "red") +
...
# strawberry combined with pesticide
```{r}
Pest <- read.csv("Pesticides.csv")
Pest <- Pest[Pest$Pesticide != "",]
strawb_insect <- strawb1 %>% filter((chemical.type=="INSECTICIDE"))
...
straw_health<- strawb_insect %>% inner_join(Pest,by="chemical")
